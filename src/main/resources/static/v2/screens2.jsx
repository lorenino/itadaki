// Upload, Correction, History, Profile screens

function Upload({T,onCancel,onAnalyzed,mobile}){
  const [stage,setStage]=useState('idle'); // idle|preview|loading|error
  ...
  const pick=f=>{if(!f)return;const r=new FileReader();r.onload=e=>{setImg(e.target.result);setStage('preview');};r.readAsDataURL(f);};
  const usePlaceholder=()=>{setImg('PLACEHOLDER');setStage('preview');};
  const analyze=()=>{setStage('loading');setProg(0);
    let p=0;const t=setInterval(()=>{p+=4+Math.random()*6;if(p>=100){clearInterval(t);setProg(100);setTimeout(()=>onAnalyzed({
      id:'new',name:'Ramen shoyu au porc chashu',ing:['nouilles ramen','bouillon shoyu','porc chashu','œuf mollet','pousses de bambou','oignon vert'],
      portion:'grand',kMin:720,kMax:880,conf:71,date:new Date().toISOString(),meal:'Dîner',seed:13,img
    }),300);}else setProg(p);},90);};
  ...
  if(stage==='loading'){
    const variants=[
      ...
      <div key="v2" style={{display:'flex',flexDirection:'column',alignItems:'center',gap:18,width:'100%',maxWidth:360}}>
        ...
        <div style={{width:'100%',background:T.bgAlt,borderRadius:18,padding:18,fontFamily:'JetBrains Mono,monospace',fontSize:13,color:T.inkMuted}}>
          {['→ plat détecté','→ ingrédients: 6/7','→ portion estimée','→ calories…'].slice(0,Math.min(4,Math.ceil(prog/25))).map((l,i)=><div key={i} style={{padding:'3px 0',opacity:1,animation:'rise .3s ease'}}>{l}</div>)}
      ...
      // v3 — orbiting dots + wave
      <div key="v3" style={{display:'flex',flexDirection:'column',alignItems:'center',gap:22,width:'100%',maxWidth:360}}>
        <div style={{position:'relative',width:160,height:160}}>
          {[0,1,2,3,4].map(i=>{
            const a=(Date.now()/20+i*72)*(Math.PI/180);
            return <div key={i} style={{position:'absolute',left:'50%',top:'50%',width:10,height:10,borderRadius:'50%',background:i%2?T.accent:T.matcha,transform:`translate(-50%,-50%) rotate(${i*72}deg) translateY(-60px)`,animation:`breathe 1.${4+i}s ease-in-out infinite`,animationDelay:`${i*.1}s`}}/>;
          })}
    ...
    return <div style={{minHeight:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 24px',background:T.bg,color:T.ink,textAlign:'center'}}>
      {variants[loadVar]}
  ...
  return <div style={{maxWidth:760,margin:'0 auto',padding:mobile?'8px 4px':'0'}}>
    ...
    {stage==='preview'&&<>
      <div style={{borderRadius:24,overflow:'hidden',background:T.bgAlt,position:'relative',aspectRatio:'4/3'}}>
        {img==='PLACEHOLDER'?<Dish seed={13} style={{width:'100%',height:'100%'}} rounded={0}/>:
...
function History({T,meals,onMeal,mobile}){
  ...
  const groups={};
  meals.forEach(m=>{if(!m.date)return;const k=m.date.slice(0,10);(groups[k]=groups[k]||[]).push(m);});
  const days=Object.keys(groups).sort().reverse();
  const dayName=k=>{const d=new Date(k+'T12:00:00');const td=new Date().toISOString().slice(0,10);const yd=new Date();yd.setDate(yd.getDate()-1);const y=yd.toISOString().slice(0,10);if(k===td)return 'Aujourd\'hui';if(k===y)return 'Hier';return d.toLocaleDateString('fr-FR',{weekday:'long',day:'numeric',month:'long'});};
...
function Profile({T,user,onLogout,mobile,dark,setDark}){
  ...
  return <div style={{maxWidth:620,margin:'0 auto'}}>
    <div style={{display:'flex',alignItems:'center',gap:18,marginBottom:28}}>
      <div style={{width:74,height:74,borderRadius:'50%',background:T.accent,color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'"Fraunces",serif',fontSize:34,fontStyle:'italic',fontWeight:500,flexShrink:0,boxShadow:`0 10px 28px -10px ${T.accent}`}}>{user.username[0].toUpperCase()}</div>
...
function Row({T,label,value}){
  return <div style={{display:'flex',justifyContent:'space-between',padding:'13px 0',borderBottom:`1px solid ${T.hairline}`,fontFamily:'Inter,system-ui',fontSize:14}}>
    <span style={{color:T.inkMuted}}>{label}</span><span style={{color:T.ink,fontWeight:600}}>{value}</span>
  </div>;
}
function RowBtn({T,label,sub,danger,onClick}){
  return <button onClick={onClick} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'13px 0',borderBottom:`1px solid ${T.hairline}`,border:'none',background:'transparent',cursor:'pointer',textAlign:'left',fontFamily:'Inter,system-ui'}}>
    <div>
